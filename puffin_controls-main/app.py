...
def get_current_shot_folder():
    ...
    # Get the highest shot number for today
    if existing_shots:
        current_shot = max(existing_shots)
    else:
        current_shot = 1
        # Create first shot folder for today
        shot_folder = os.path.join(SHOT_BASE_DIR, f"{prefix}{current_shot:02d}")
        os.makedirs(shot_folder, exist_ok=True)
        print(f"Created initial shot folder for today: {shot_folder}")
        ...
        if scope:
            ...
            for ch_name, ch_settings in channels.items():
                if ch_name.startswith('CH'):
                    ...
                    display = ch_settings.get('display', ch_settings.get('on/off', 'on'))  # Support both 'display' and 'on/off' keys
                    coupling = ch_settings.get('coupling')
                    ...
                    if vdiv is not None:
                        try:
                            ...
                            if coupling:
                                scope.set_channel_coupling(ch_num, coupling)
                            applied_settings[f'channel_{ch_num}'] = {'vdiv': vdiv, 'display': display, 'coupling': coupling}
                        except Exception as e:
...
def api_upload_json():
    """Upload a JSON file and save it as device settings.json in current shot folder"""
    try:
        if 'file' not in request.files:
            return jsonify({"error": "No file part in request"}), 400
...
def api_write_scope_to_json():
    ...
    try:
        ...
        def read_scope_settings():
            ...
            for ch in range(1, 5):
                try:
                    ...
                    coupling_resp = scope.query(f":CHANnel{ch}:COUPling?")
                    ...
                    coupling = coupling_resp.strip().upper() if coupling_resp else "DC"
        ...
        # Read existing JSON file and preserve structure
        shot_folder = get_current_shot_folder()
        json_file_path = os.path.join(shot_folder, "device settings.json")
...
def api_set_scope_from_json():
    ...
    try:
        # Read the JSON file from current shot folder
        shot_folder = get_current_shot_folder()
        json_file_path = os.path.join(shot_folder, "device settings.json")
        ...
        if scope:
            ...
            for ch_name, ch_settings in channels.items():
                if ch_name.startswith('CH'):
                    ...
                    display = ch_settings.get('display', ch_settings.get('on/off', 'on'))  # Support both keys
                    coupling = ch_settings.get('coupling')
                    ...
                    try:
                        ...
                        if coupling:
                            scope.set_channel_coupling(ch_num, coupling)
                        applied_settings[f'channel_{ch_num}'] = {'vdiv': vdiv, 'offset': offset, 'display': display, 'coupling': coupling}
                    except Exception as e:
                        return jsonify({"error": f"Failed to set channel {ch_num}: {e}"}), 400
...
# Plotting disabled - matplotlib not available
...
def acquire_all_channels_worker(scope, task_id):
    ...
    try:
        ...
        csv_file = os.path.join(shot_folder, f"scope1_waveform_{date_shot}.csv")
        ...
        # Create plot of waveform data and save as PDF
        tasks[task_id]["status"]["progress"] = "Creating waveform plot..."
        pdf_file = os.path.join(shot_folder, f"scope1_waveform_{date_shot}.pdf")
        ...
        try:
            ...
            # Add overall title
            fig.suptitle(f'Oscilloscope Waveform - Shot {date_shot}', fontsize=14, fontweight='bold')
            
            plt.tight_layout()
            plt.savefig(pdf_file, format='pdf', dpi=300, bbox_inches='tight')
            plt.close(fig)
            
            print(f"✓ Waveform plot saved to {pdf_file}")
            
        except Exception as e:
            print(f"Warning: Could not create waveform plot: {e}")
            traceback.print_exc()
            # Don't fail the entire acquisition if plotting fails
        
        # Prepare next shot folder for the next acquisition
        try:
            next_shot_folder = prepare_next_shot_folder()
        except Exception as e:
            print(f"Warning: Could not prepare next shot folder: {e}")
        
        # Update status to complete
        result_data = {
            "csv_file": os.path.basename(csv_file),
            "pdf_file": os.path.basename(pdf_file) if os.path.exists(pdf_file) else None,
                    ...
                    def read_current_settings():
                        ...
                        for ch in range(1, 5):
                            try:
                                ...
                                coupling = scope.query(f":CHANnel{ch}:COUPling?")
                                ...
                                settings['channels'][f'CH{ch}'] = {
                                    ...
                                    'coupling': coupling.strip().upper() if coupling else 'DC'
                                }
                            except Exception as e:
...
def api_download(filename):
    """Download CSV file from current shot folder"""
    try:
